...
public class DataFactory implements Data {
    ...
    public List<User> getAllProjectUsersByUserId(final String userId) {
        return process(new Handler<List<User>>() {
            @Override
            public List<User> handle(Connection connection, QueryRunner qr) throws SQLException {
                StringBuilder sql = new StringBuilder("select u.id,u.nickname,avatar from "+TableNames.USER+" u \n" +
                        "where u.id in (\n" +
   ...
   @Override
    ...
    public List<InterfaceFolder> getFoldersByProjectId(String projectId) {
        return process(new Handler<List<InterfaceFolder>>() {
            @Override
            public List<InterfaceFolder> handle(Connection connection, QueryRunner qr) throws SQLException {
                StringBuilder sql = new StringBuilder("select * from ").append(TableNames.INTERFACE_FOLDER).append(" order by createtime asc");
                return qr.query(connection,sql.toString(),new BeanListHandler<>(InterfaceFolder.class));
            }
        });
    }
    ...
    public List<Interface> getInterfacesByProjectId(String projectId) {
        return process(new Handler<List<Interface>>() {
            @Override
            public List<Interface> handle(Connection connection, QueryRunner qr) throws SQLException {
                StringBuilder sql = new StringBuilder("select * from ").append(TableNames.INTERFACES).append(" order by createtime asc");
                return qr.query(connection,sql.toString(),new BeanListHandler<>(Interface.class));
            }
        });
    }
    ...
    public int findPassword(final String id, final String email, final String password) {
        return process(new Handler<Integer>() {
            ...
            public Integer handle(Connection connection, QueryRunner qr) throws SQLException {
                ...
                int rs = qr.update(connection,sql,email,newPassword);
